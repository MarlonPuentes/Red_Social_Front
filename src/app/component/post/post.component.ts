...
export class PostComponent implements OnInit, OnDestroy {
	...
	openPostDeleteConfirmDialog(): void {
		const dialogRef = this.matDialog.open(ConfirmationDialogComponent, {
			data: 'Quieres eliminar esta publicacion permanentemente?',
			autoFocus: false,
			width: '500px'
		});
	...
	deletePost(postId: number, isTypeShare: boolean): void {
		const dialogRef = this.matDialog.open(WaitingDialogComponent, {
			data: 'Por favor, espere mientras borramos la publicación.',
			width: '500px',
			disableClose: true
		});

		console.log(isTypeShare)
			...
			this.postService.deletePost(postId, isTypeShare).subscribe({
				next: (response: any) => {
					...
					this.matSnackbar.openFromComponent(SnackbarComponent, {
						data: 'Publicacion eliminada exitosamente.',
