...
export class SearchDialogComponent implements OnInit, OnDestroy {
	...
	searchUser(currentPage: number): void {
		if (!this.fetchingResult) {
			if (this.key.value.length >= 3) {
				...
				this.matSnackbar.openFromComponent(SnackbarComponent, {
					data: 'La clave de búsqueda debe tener entre 3 y 64 caracteres.',
					panelClass: ['bg-danger'],
					duration: 5000
				});
			}
	...
	openFollowConfirmDialog(userResponse: UserResponse): void {
		const dialogRef = this.matDialog.open(ConfirmationDialogComponent, {
			data: `Quieres seguir a ${userResponse.user.firstName + ' ' + userResponse.user.lastName}?`,
			autoFocus: false,
			maxWidth: '500px'
		});
					...
					this.subscriptions.push(
						this.userService.followUser(userResponse.user.id).subscribe({
							next: (response: any) => {
								...
								this.matSnackbar.openFromComponent(SnackbarComponent, {
									data: `Ahora sigues a ${userResponse.user.firstName + ' ' + userResponse.user.lastName}.`,
									duration: 5000
								});
							},
							error: (errorResponse: HttpErrorResponse) => {
	...
	openUnfollowConfirmDialog(userResponse: UserResponse): void {
		const dialogRef = this.matDialog.open(ConfirmationDialogComponent, {
			data: `Quieres dejar de seguir a ${userResponse.user.firstName + ' ' + userResponse.user.lastName}?`,
			autoFocus: false,
			maxWidth: '500px'
		});
					...
					this.subscriptions.push(
						this.userService.unfollowUser(userResponse.user.id).subscribe({
							next: (response: any) => {
								...
								this.matSnackbar.openFromComponent(SnackbarComponent, {
									data: `Ya no sigues a ${userResponse.user.firstName + ' ' + userResponse.user.lastName}.`,
									duration: 5000
								});
							},
							error: (errorResponse: HttpErrorResponse) => {
